...
contract AsciiPunks {
  ...
  uint public constant PRICE = 300000000000000000;
  ...
  */
  ...
  function createPunk(uint seed) external payable returns (string memory) {
  ...
  function draw(uint id) public view returns (string memory) {
    uint rand = uint(keccak256(abi.encodePacked(idToSeed[id])));
